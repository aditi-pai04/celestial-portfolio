...
export default function Contact() {
  ...
  return (
    <section
      id="signal"
      className="w-full max-w-4xl mx-auto py-32 px-8 mt-16 border-t border-black/10 dark:border-white/10 transition-all duration-700"
    >
      {/*SYSTEMS MODE*/}
      <div className="dark:hidden space-y-8 font-mono">
        <div className="space-y-2">
          <h2 className="text-2xl font-bold uppercase tracking-[0.3em] text-accent">
            [Open_Uplink]
          </h2>
          <p className="text-[10px] opacity-50 uppercase tracking-widest">
            Establish a direct connection to the MTS-1 node.
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <input
              ...
              className="bg-black/5 border border-black/20 p-4 focus:border-accent outline-none text-xs"
            />
            <input
              name="email"
              required
              type="email"
              placeholder="RETURN_PATH"
              className="bg-black/5 border border-black/20 p-4 focus:border-accent outline-none text-xs"
            />
          </div>
          <textarea
            name="message"
            required
            placeholder="ENCODE_MESSAGE_BODY..."
            rows={5}
            className="w-full bg-black/5 border border-black/20 p-4 focus:border-accent outline-none text-xs"
          />
          <button
            type="submit"
            disabled={status === "SENDING"}
            className="w-full bg-black text-white py-4 font-bold uppercase tracking-[0.4em] hover:bg-accent transition-all text-xs disabled:opacity-50"
          >
            {status === "SENDING"
              ? "TRANSMITTING..."
              : status === "SUCCESS"
                ? "DONE. SIGNAL_RECEIVED"
                : "> EXECUTE_TRANSMIT"}
          </button>
        </form>
      </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-10 max-w-lg mx-auto">
          ...
          <div className="flex flex-col items-center gap-8 pt-4">
            ...
            <p className="text-[9px] opacity-40 uppercase tracking-[0.4em] italic text-accent">
              {status === "SUCCESS"
                ? "Signal reached the stars."
                : status === "ERROR"
                  ? "Uplink failed. Try again."
                  : "Geometry in motion."}
            </p>
          </div>
        </form>
